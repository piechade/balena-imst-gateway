...
[![](https://www.balena.io/deploy.png)](https://dashboard.balena-cloud.com/deploy?repoUrl=https://github.com/piechade/balena-imst-gateway)
...
**`GW_RESET_PIN`** | `INT` | Pin number that resets (Raspberry Pi header number) | 5
**`GW_RESET_GPIO`** | `INT` | GPIO number that resets (Broadcom pin number, if not defined, it's calculated based on the GW_RESET_PIN) | 5
